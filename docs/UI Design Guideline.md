...
一般操作不得在背景排程或生成只供左側 Job List 使用的 Quick Thumbnail、Active Job Thumbnail 或 Hidden iframe Thumbnail Queue，也不得更新已移除的 Thumbnail DOM。這不代表整套 Thumbnail 系統已刪除：完整專案／Batch 正式 PNG 建立的 `job.thumbnail`、Project State thumbnail 欄位與 import 相容仍保留。下載單張暫存不再執行 on-demand thumbnail capture，且輸出 JSON 不包含 `jobs[].thumbnail`；素材與版面等真正暫存資料仍完整保留。
