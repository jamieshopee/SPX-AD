...
Phase 3 macOS Packaging Bug Fix：Code Commit `781df79c232a9644cc0bd69653e390ef70d12964`（`fix: launch macOS helper through LaunchAgent`）修正 PKG 安裝完成啟動來源。原 `postinstall` 直接 `/usr/bin/open` App，導致 Helper 繼承 PackageKit `PKInstallSandbox` environment，sandbox 刪除後 `tempfile.mkdtemp()` 失敗並使 `/execute` 回傳 HTTP 400 `manifest_invalid`。修正保留 LaunchAgent bootstrap，改以非強制 kickstart 啟動 GUI user domain 中的 `com.spxad.helper` job；不使用 `kickstart -k`，不新增 `KeepAlive`，`RunAtLoad`、Quit 與 Login Startup 行為不變。Clean Install、正常 Helper environment、正式 GitHub Pages → Helper → Photoshop → Processed PNG 與 Jamie Manual Validation 均 PASS。此 Bug Fix 未修改 Windows Packaging，Phase 4、Phase 5 仍未開始。
