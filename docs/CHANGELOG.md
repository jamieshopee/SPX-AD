...
- Phase 3 macOS Packaging、Phase 4 Update + Uninstall 與 Phase 5 Final Validation 尚未完成。
...
- **SPX Helper Runtime Productization Phase 2 Windows Packaging 完成（功能 Commit `9240504`）**：新增 PyInstaller executable bundle 與 WiX Toolset SDK 5.0.2 per-machine MSI；Windows Packaging configuration、MSI Build／Install 與 GitHub Pages → Helper → Photoshop → Processed PNG 均 PASS。下一步為 Phase 3 — macOS Packaging。
...
- **SPX Helper Core 完成（功能 Commit `9a71794`）**：新增正式 localhost 邊界、Origin／CORS、單一實例邊界與既有 RuntimeCore Integration；macOS／Windows Jamie Manual Validation 已完成。Windows installer／auto-start／packaging 已由 Phase 2 完成；macOS Packaging、Update／Uninstall 與 Final Validation 仍待後續 Phase。
