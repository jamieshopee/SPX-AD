...
Code Commit?`f37d37e13b0770acab0559dae318576c82458971`
Commit subject?`fix: guard WebP content with PNG extension`

### Fixed

- Windows Photoshop 2026 ????? `.png`?????? WebP?`RIFF....WEBP` magic bytes??????? `app.open()` ????? Crash?BEX64 / `0xc0000409`???? Crash ???? JSX ??? per-item catch????????? Run Report?
- `tools/photoshop/remove-background.jsx` ? `processItem()` ? `sourceFile.exists` ?????`app.open(sourceFile)` ??????????? Guard?? `.png` ??? magic bytes ? `RIFF....WEBP` ??????????????????????? Photoshop ???
- Guard ???? per-item catch?Run Report ?? `status: "error"`???????????? Browser?Runtime ??????????? `background_removal_failed`?Review Workspace ??????????????????

### Validation

- Jamie Manual Validation PASS?Windows Photoshop 2026 ?? Crash????????????? `background_removal_failed`?????????????????
- macOS ? Windows ??????? JSX ???????????
...
- Packaging Status：SPX Helper 正式交付版本已修正為 `0.6.1`，重新完成 macOS Local Packaging 並建立 `SPX Helper-0.6.1.pkg`；本輪不安裝新版 PKG。Repo、App Bundle 與 PKG Payload 內的 `remove-background.jsx` SHA-256 完全一致；既有 Helper 啟動、Existing Transparency、JPG、不透明 PNG、Logo、Runtime Contract 與 Packaging Manual Validation 結果均維持 PASS。

## SPX Helper macOS Local Packaging 0.6.1 - 2026-07-28

Status：**Completed — Local Packaging／Jamie Manual Validation PASS**

- Version Correction：原先將本次 SPX Helper 設為 `0.5.5`，但該編號與既有 GitHub `v0.5.5` Release 衝突，且目前最新既有 Release 已為 `v0.6.0`。因此使用新增修正 Commit 將正式版本改為 `0.6.1`，不 reset、amend、rebase 或改寫任何已 Push Git 歷史；`CFBundleShortVersionString`、`CFBundleVersion` 與 PKG metadata 同步為 `0.6.1`。
- Deliverables：重新建立 `SPX Helper.app` 與 `SPX Helper-0.6.1.pkg`；本輪不安裝新版 PKG。既有安裝與 Helper 啟動驗證結果維持 PASS。
...
- Scope Boundary：未修改 Runtime、Adapter、Browser、Review Workspace、Workflow、UI、Installer Flow、Signing 或 Notarization；本次尚未 Push、建立 `v0.6.1` Tag 或 GitHub Release。
