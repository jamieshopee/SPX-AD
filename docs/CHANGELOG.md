...
- 本 Phase 不包含 Installer、Windows MSI、macOS PKG、登入自動啟動、Update、Uninstall、Signing、Notarization、Authenticode 或 Final Validation。Phase 2～5 均未誤標為完成。
...
- **SPX Helper Runtime Productization Phase 1 Foundation 完成（功能 Commit `51c4828`）**：新增 Product Host、Running／Working／Attention、單一 Helper Instance、固定 Tray／Menu Bar、Quit 與 Restart；Browser／Platform Validation 與 Jamie Manual Validation 均 PASS。Phase 2～5 尚未完成。
- **SPX Helper Core 完成（功能 Commit `9a71794`）**：新增正式 localhost 邊界、Origin／CORS、單一實例邊界與既有 RuntimeCore Integration；macOS／Windows Jamie Manual Validation 已完成。完整 installer／auto-start／update／signing／packaging 仍待後續 Proposal。
