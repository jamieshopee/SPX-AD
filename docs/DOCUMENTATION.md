...
- SPX Helper Runtime Productization — Phase 3 macOS Packaging（PyInstaller `SPX Helper.app`、macOS PKG、Applications、LaunchAgent、Login Startup、Menu Bar、macOS Validation；功能 Commit `ee55dd527a00361f1155ba45713ff2ce3957b06c`；安裝啟動 Bug Fix Commit `781df79c232a9644cc0bd69653e390ef70d12964`；Developer ID／Notarization 為 Credential-dependent validation）
...
- `匯入暫存` 只接受 JSON，支援單檔或多檔；每份 JSON 必須只包含一個 Job，同批依完整檔名 Natural Sort 後 append 至既有 Job List 尾端，不執行 Workspace replace。
- 每個 imported Job 會恢復來源 JSON 的 Placement、Template、Style、`layoutState`／`layoutStates`；匯入後不需要重新 Import Processed Folder。
- Imported Job 在 `selectStyle()` 更新並正規化 `job.styleId` 後，會同步 runtime-only `_importedRenderContext.styleId`；因此切換 Job 再返回時，`selectJob()` 讀到的是使用者最後選擇的 Style，而不是匯入快照的舊值。此規則只涵蓋 Style，不改變 Placement、Template、Import schema、Render Context 或普通 CSV Job。
- Raw／processed assets、pipeline identity 與 embedded export filename 在 commit 前執行 Collision Preflight；同 normalized filename 且同 data URL 可共用，同名不同內容則整批拒絕。
- 匯入採 Atomic Append；任何檔案失敗時，不留下部分 Job、asset、pipeline 或 active state。普通 CSV Workspace 行為維持不變。
- 匯入後 Review Workspace、Main Canvas、Thumbnail、Batch resolver 都應使用 restored latest processed image。

### Download Complete Project single-state Restore

- 完整專案 ZIP 根目錄只包含每個成功 Job 同 basename 的 PNG 與 version 5 single-state JSON 配對。
- 每份 JSON 只包含一個 Job、不包含 `jobs[].thumbnail`，並沿用正式「下載單張暫存」格式保存該 Job 的 assets、processed image 與完整還原資料。
- 從 ZIP 取出的任一 JSON 可直接透過既有「匯入暫存」重建該 Job 的 runtime `processedAssetIndex`。
- 不輸出單一 Project JSON，也不建立 Assets、Processed、Thumbnail、Hidden、Manifest 或其他子資料夾。
- 從 ZIP 取出的逐 Job JSON 可透過 JSON-only「匯入暫存」單選或多選 append；ZIP 本身不是匯入格式。

### Download Complete Project

- 使用者主要下載流程為「下載完整專案」。
- ZIP 內每個成功 Job 都有同 basename 的 PNG 與 single-state JSON；只有 PNG／JSON 都成功才加入 ZIP。
...
- Existing Transparency Skip（Code Commit `af30a4106b82e5661ae72d768f6af1141ad632fb`）：非 Logo 素材仍由 Photoshop 開圖，再依實際影像內容檢查有效透明背景；命中時只略過 Remove Background，仍 Save PNG、寫 success Run Report 並完成 Runtime Result、Browser Fetch、`Processed/` 寫入、Matching 與 Review Workspace。不透明 PNG／JPG、Logo copy、fallback、failure contract、First Run／Needs Rerun 與後續 Download／Render／Import／Export 均維持既有契約。Jamie Manual Validation PASS，Runtime Contract 60/60 PASS。
- Scope Boundary：上述功能只修改 `tools/photoshop/remove-background.jsx`，未修改 Browser、Python Runtime、macOS／Windows Adapters、Manifest／Runtime API、Review Workspace、Asset Resolver、Download、Import／Export、Render Context 或 Packaging Flow。SPX Helper 正式交付版本已修正為 `0.6.1`，重新完成 macOS Local Packaging；`SPX Helper-0.6.1.pkg` 包含新版 JSX，本輪不安裝新版 PKG。原 `0.5.5` 與既有 GitHub Release 編號衝突，版本以新增修正 Commit 更正而未改寫已 Push 歷史；既有安裝與 Manual Validation 結果維持 PASS，Bundle ID、Package ID 與 LaunchAgent 不變。
