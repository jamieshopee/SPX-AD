...
注意命名：「Photoshop Rerun Automation」（已完成，上表內）與「Photoshop Automation」（目前 Active Phase，Proposal 階段，尚未完成，不在上表內）是兩個不同 Phase，不得混用或誤植。
...
After Review Workspace UI Upgrade v0.4.5, the planned phase order is:
...
- Photoshop Automation is the current Active Phase (Proposal stage) and is a required prerequisite for AI Workflow. It must start with Product Proposal / Proposal Audit before implementation, and must not redesign Review Workspace UI, Navigator, Dynamic Inspector, Decision Area, Completion Screen, Crop, Eraser, Canvas, Thumbnail, Batch, `layoutStates`, Approved Asset Resolver, Project State schema, or Review Decision Model.
- AI Workflow is paused (Draft / Paused pending Photoshop Automation) until Photoshop Automation is completed; it must not enter Proposal Freeze / Implementation Proposal in the meantime. Once resumed, AI Workflow may only automate the background asset-processing flow; it must not redesign Review Workspace UI, Navigator, Dynamic Inspector, Decision Area, or Completion Screen.
...
## Photoshop Automation Rules

Photoshop Automation 是目前 Active Phase（Proposal 階段，尚未 Freeze、尚未 Coding），是 AI Workflow 的必要前置依賴。Photoshop Automation 是「Photoshop 端」的自動處理能力，不是 Control Center orchestration；Control Center 端的自動化屬於 AI Workflow（見下方責任切分）。

- 前提：所有使用者已安裝 Adobe Photoshop，並會在使用控制台前自行開啟 Photoshop。本 Phase 不負責自動安裝、自動啟動或自動關閉 Photoshop。
- 負責：定義 Photoshop Ready Contract，讓已由使用者開啟的 Photoshop 可準備接收工作；接收既有格式的 Photoshop Job Manifest / Rerun Manifest；呼叫並沿用既有 `remove-background.jsx` 去背核心；背景執行批次素材處理；輸出既有格式的 processed assets；產生 Progress / Completion / Partial Failure / Failure 狀態；產生或維護 Run Report / Status Contract；評估現有一次性 command → AppleScript → JSX Runner 如何擴充。
...
- 現況：現有一次性 AppleScript + JSX Runner（`tools/photoshop/`）將於 Proposal 中評估如何擴充；Watcher、Heartbeat、固定 Runtime Folder、啟動腳本等仍是 Proposal 選項，尚未 Freeze，不得寫成已確定實作或已完成行為。
...
- 負責：使用既有 `buildPhotoshopJobManifest` / `buildPhotoshopRerunManifest` 建立 Manifest；透過 Photoshop Automation 已確立的 Contract 送出工作；執行 Photoshop Ready Check 的 Control Center 端流程（顯示「請先開啟 Photoshop」與「重新檢查」）；顯示並鎖定 Processing Mode；讀取 Photoshop Automation 回報的狀態；沿用既有 `importProcessedAssets()` 自動掃描與 Matching / Import；自動開啟既有 Review Workspace；串接第一輪全部素材審閱與 Rerun 後的待重新去背第二輪審閱、返回控制台。
...
- 「重新去背素材（N）」目前只呼叫既有 `exportPhotoshopRerunManifest` callback，尚未自動化。責任精確拆分：Photoshop 自動處理能力／Ready Check／Progress／Run Result 屬於 Photoshop Automation；Control Center 自動 Import／Processing Mode／自動開啟第二輪 Review 屬於 AI Workflow。本 Roadmap 不包含「系統自動啟動 Photoshop App」；使用者需自行先開啟 Photoshop，系統只負責 Ready Check。
...
## AI Workflow Documentation Sync（Future，尚未執行）

AI Workflow Phase 完成後，必須再次更新以下文件（現在僅為提醒，尚未執行）：
...
在 AI Workflow 完成並通過 Jamie Manual Validation 前，不得將 Future AI Workflow Target 內容標記為 Completed。
