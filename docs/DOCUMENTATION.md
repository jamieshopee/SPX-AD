...
- Scope Boundary：上述功能只修改 `tools/photoshop/remove-background.jsx`，未修改 Browser、Python Runtime、macOS／Windows Adapters、Manifest／Runtime API、Review Workspace、Asset Resolver、Download、Import／Export、Render Context 或 Packaging。已安裝的 `/Applications/SPX Helper.app` 尚未重新 Packaging，故尚未包含新版 JSX；未來重新 Packaging 後才會帶入。
