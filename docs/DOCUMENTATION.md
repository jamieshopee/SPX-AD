...
- AI Workflow is the next planned phase and must start with Product Proposal / Proposal Audit before implementation. AI Workflow may only automate the background asset-processing flow; it must not redesign Review Workspace UI, Navigator, Dynamic Inspector, Decision Area, or Completion Screen.
...
- 「重新去背素材（N）」目前只呼叫既有 `exportPhotoshopRerunManifest` callback；Background Runner、Photoshop 自動啟動、自動 Import Processed Result、自動第二輪 Review 屬於 AI Workflow Phase，尚未實作。
