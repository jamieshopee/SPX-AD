...
預計下一個 Git Tag／GitHub Release：`v0.6.1`（尚未建立）。
...
- SPX Helper Runtime Productization — Phase 3 macOS Packaging：**Completed**。功能 Commit `ee55dd527a00361f1155ba45713ff2ce3957b06c`（`feat: add Phase 3 macOS packaging`）新增 PyInstaller `SPX Helper.app` 與正式 macOS PKG build pipeline，安裝到 `/Applications/SPX Helper.app`，並以 `/Library/LaunchAgents/com.spxad.helper.plist` 提供 `RunAtLoad = true`、不使用 `KeepAlive` 的 Login Startup。Product Host version 現為 `0.6.1`，Local PKG 為 `SPX Helper-0.6.1.pkg`。原 `0.5.5` 與既有 GitHub `v0.5.5` Release 編號衝突，且最新既有 Release 已為 `v0.6.0`；版本以新增修正 Commit 更正為 `0.6.1`，不改寫已 Push Git 歷史。本輪只重新 Local Packaging、不安裝新版 PKG；既有安裝、Helper 啟動與 Existing Transparency／JPG／不透明 PNG／Logo／Runtime Contract／Packaging Manual Validation 均維持 PASS。Bundle ID、Package ID 與 LaunchAgent 不變。Developer ID Application signing、Developer ID Installer signing 與 Apple Notarization 因缺少有效 signing identities／notary credentials，仍是 Credential-dependent validation，不得宣稱 PASS。本次尚未 Push、建立 `v0.6.1` Tag 或 GitHub Release。
...
Photoshop Automation、AI Workflow、Render Context & Export Workflow、QR Code、SPX Helper Core，以及 Runtime Productization Phase 1 Foundation、Phase 2 Windows Packaging、Phase 3 macOS Packaging 均已完成。Phase 3 功能 Commit 為 `ee55dd527a00361f1155ba45713ff2ce3957b06c`；Product Host version 現為 `0.6.1`，Local 交付檔為 `SPX Helper-0.6.1.pkg`。原 `0.5.5` 與既有 GitHub Release 編號衝突，已用新增修正 Commit 更正而未改寫已 Push 歷史；目前最新既有 Release 為 `v0.6.0`，預計新 Tag／Release 為 `v0.6.1`，本次尚未建立。Existing Transparency、JPG、不透明 PNG、Logo、Runtime Contract 與 Packaging 均 PASS；Bundle ID、Package ID 與 LaunchAgent 不變。本次尚未 Push。下一步是 **Phase 4 — Update + Uninstall（Not Started）**；Phase 5 Final Validation 尚未開始。目前不宣稱支援所有 Photoshop 版本，也不宣稱 Developer ID／Notarization PASS。

### Phase 3 macOS Packaging Validation Status

- 正式產品資訊：Product Name `SPX Helper`、Bundle Identifier `com.spxad.helper`、Package Identifier `com.spxad.helper.pkg`、Product Version `0.6.1`、安裝位置 `/Applications/SPX Helper.app`、Local PKG `SPX Helper-0.6.1.pkg`。
...
- Jamie Manual Validation：既有 Fresh Install、LaunchAgent、Package Receipt、Menu Bar、Running、Open SPX BN Generator、無 Dock Icon、無 Terminal Window、正式 GitHub Pages → Helper → Photoshop → Processed PNG、Restart、Quit、Login Startup 均維持 PASS；正式交付版本已修正為 `0.6.1` 並重新執行 Local Packaging，本輪不安裝新版 PKG；Existing Transparency、JPG、不透明 PNG、Logo、Runtime Contract 與 Packaging 驗證結果均維持 PASS。
