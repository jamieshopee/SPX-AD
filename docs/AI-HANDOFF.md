...
- Related functionality：Code Commit `f37d37e13b0770acab0559dae318576c82458971`（`fix: guard WebP content with PNG extension`）；Docs Commit `eb8e67ce09405bda4d22de766faf14138afca271`（`docs: document WebP PNG mismatch guard`）。
...
- Version／Deliverables：Product Host、App bundle、PKG metadata、validators 與 receipt expectation 均同步為 `0.6.2`；產物為 `SPX Helper.app` 與 `SPX Helper-0.6.2.pkg`。PKG SHA-256：`32b55924db02074c35292a03edd564f3b98032e74565e4f1f310c8fbe50dd596`。
- Build／Artifact Validation：macOS static packaging validation、PyInstaller App build、structural codesign verification、component plist、`pkgbuild`、`productbuild` 與 PKG expansion／metadata／payload validation 全部 PASS；Product Host lifecycle regression 29 項 PASS。
- Jamie Manual Validation：安裝、Menu Bar、Version `0.6.2`、無 Dock icon、無 Terminal window、`/ready`、LaunchAgent、package receipt `0.6.2`、Photoshop 2026 去背、Processed PNG／report／Matching／Review 全部 PASS。WebP content + `.png` filename 正確回報 `background_removal_failed`，後續素材繼續完成。
- JSX Inclusion：Repository、App bundle 與 PKG payload 的 `remove-background.jsx` byte-for-byte 一致，共同 SHA-256 為 `796680519c488f53587d385a06785d3aac6059e4530953c598ec6e56ee8e89a7`。
- Identity／Install Contract：Bundle ID `com.spxad.helper`、Package ID `com.spxad.helper.pkg`、LaunchAgent `com.spxad.helper`、`/Applications/SPX Helper.app`、`/Library/LaunchAgents/com.spxad.helper.plist`、`RunAtLoad = true`、無 `KeepAlive`、component `upgrade` 與不可 relocation 均維持既有值。
- Local Build Limitation：本機無有效 Developer ID Application／Installer identity；App 為 ad-hoc signature、PKG unsigned，未執行 Developer ID signing、notarization 或 stapling。Gatekeeper rejection 是 local unsigned artifact 的預期結果，不得宣稱正式簽章或 notarization PASS。PyInstaller 僅有既有條件式、跨平台或 optional-module warnings；SDK alignment 是 informational；無 Build Error。
- Scope Boundary：Packaging Commit 未修改共用 JSX、Browser、Runtime Contract、macOS Adapter、Manifest、Pipeline State、Review Workspace、UI、`qrcode-demo`、Windows MSI、Auto Update 或 Upgrade／Uninstall Phase 範圍。
- Delivery Status：Jamie macOS Manual Validation PASS；尚未 Push；尚未建立 Tag／Release；亦未上傳 PKG。

## ?????Windows Photoshop 2026 WebP?PNG ?????????2026-07-30?

- Code Commit?`f37d37e13b0770acab0559dae318576c82458971`?`fix: guard WebP content with PNG extension`??
- Root Cause?Windows Photoshop 2026 ???? `.png`????? WebP?magic bytes `RIFF....WEBP`??????? `app.open()` ??? Crash?BEX64 / `0xc0000409`???? Crash ???? JSX????? per-item catch ???? Run Report?
- ?????`tools/photoshop/remove-background.jsx` ? `processItem()` ? `app.open(sourceFile)` ????? Guard ???????????????????????? per-item catch ?? `status: "error"`??????????
- ???????Browser ??????????? `background_removal_failed`?Review Workspace ??????????????????????????????macOS ? Windows ?? JSX ???
- Jamie Manual Validation PASS?Windows Photoshop 2026 ?? Crash???????????????????????????
- ?????? Browser?Runtime?Windows Adapter?Manifest?Pipeline State?Review Workspace?UI?Packaging?Version ? `qrcode-demo`?
...
CSV / Project State
  ↓
素材入口
  ├─ 自動去背匯入素材 → SPX Helper → Photoshop → Processed → 素材審核
...
- 非 Logo 素材由 Photoshop 開啟後，會先檢查實際影像是否已有有效透明背景；命中時只略過 Remove Background 動作，仍輸出 Processed PNG 與 success Run Report，並完整回到 Matching／Review 流程。不透明 PNG 不能因副檔名而略過，Logo 原有 copy 規則不變。
...
- 正式狀態流程為 Photoshop First Run → `FirstReview` →（有 Needs Rerun 時）Photoshop Rerun → `SecondReview` → `Completed`。Rerun 只能由 `FirstReview` 啟動；`SecondReview` 是最後一輪，不存在第三輪 Rerun；`Completed` 後不得重新開啟 Review Workspace 或重新去背。
...
- Header 的「匯入素材資料夾」是第二條獨立素材入口，不取代「自動去背匯入素材」與 Photoshop 流程。
...
預計下一個 Git Tag／GitHub Release：`v0.6.2`（尚未建立）。
...
- AI Workflow Single-pass Review：功能 Commit `8eefbb0924121f3a199c547186306c5eeb722a31`（`feat: finalize AI asset review workflow`）。Header 入口順序為匯入 CSV、匯入暫存、匯入素材資料夾、自動去背匯入素材；素材審核只在 `FirstReview`／`SecondReview` 顯示，Photoshop First Run／Rerun 實際執行時顯示「處理中（N／N）」，`Completed` 顯示不可點擊的綠色「AI 去背完成」。Review Workspace 第一輪提供核准／重新去背／之後手動換圖，第二輪只提供核准／之後手動換圖；`skipped` decision／state 不變。未完成時 Close／Esc 會被阻止；第二輪不得寫入 `needs_rerun` 或建立第三輪 Rerun；`Completed` 後不得 reopen Review。Manual Validation A1–E2 與 Code Review 全部 PASS。
...
- Review Workspace Skip Decision：功能 Commit `c2151987fae163d6e1c8af7f660f2823908846ca`（`feat: add skipped review decision`）新增 `skipped`；後續 Commit `8eefbb0924121f3a199c547186306c5eeb722a31` 將顯示名稱由「略過」改為「之後手動換圖」，decision／state 與既有行為不變。第一輪按鈕為核准／重新去背／之後手動換圖；第二輪為核准／之後手動換圖。Undo Last Decision 已從 Detail、Completion Screen 與 runtime restore flow 完整移除。`skipped` 是目前 Project 的 Terminal State，保留 Auto Next，不進 Needs Rerun、第二輪 Review 或任何 Photoshop Manifest；Project State v5 JSON 匯出／匯入保留 `status` 與 `review.decision`，手動換圖不得將其重設為 `pending`。新 CSV 會建立全新 Pipeline State，不沿用舊 Project 的 skipped。
- Review Workspace Original Fallback：Bug Fix Commit `c21c79e5e762598a35d6368fff5013ffd6ee21df`（`fix: show original for failed review assets`）。`renderDetail()` 不再讓 Processed resolver 失敗連帶阻止已成功取得的 Original 建立 Editor；`createSession()` 在有 Processed 時仍預設顯示 Processed，沒有可用 Processed 時則直接以 Original 作為初始來源。Original resolver 失敗維持既有錯誤處理，`loadSeq`／disposed 防競態、Original／Processed 切換、Session 資料結構、Navigator、Decision、Crop、Eraser 均未修改。未新增 cache、timeout、retry，也未修改 AI Workflow、Photoshop、Helper、Pipeline 或其他模組。Jamie Manual Validation PASS。
...
Photoshop Automation、AI Workflow、Render Context & Export Workflow、QR Code、SPX Helper Core，以及 Runtime Productization Phase 1 Foundation、Phase 2 Windows Packaging、Phase 3 macOS Packaging 均已完成。Phase 3 功能 Commit 為 `ee55dd527a00361f1155ba45713ff2ce3957b06c`，0.6.2 Packaging Commit 為 `3a8e925e773338491abb03b3c6cc7fdd60a0994f`；Product Host version 現為 `0.6.2`，Local 交付檔為 `SPX Helper-0.6.2.pkg`。目前最新既有 Release 為 `v0.6.0`，預計新 Tag／Release 為 `v0.6.2`，尚未建立。Photoshop 2026、WebP／PNG mismatch isolation、後續素材 continuation、Processed PNG／report／Matching／Review 與 Packaging Manual Validation 均 PASS；Bundle ID、Package ID 與 LaunchAgent 不變。尚未 Push，亦未上傳 PKG。下一步是 **Phase 4 — Update + Uninstall（Not Started）**；Phase 5 Final Validation 尚未開始。Developer ID／Notarization 仍未驗證。

### Phase 3 macOS Packaging Validation Status

- 正式產品資訊：Product Name `SPX Helper`、Bundle Identifier `com.spxad.helper`、Package Identifier `com.spxad.helper.pkg`、Product Version `0.6.2`、安裝位置 `/Applications/SPX Helper.app`、Local PKG `SPX Helper-0.6.2.pkg`。
- Login Startup：`/Library/LaunchAgents/com.spxad.helper.plist`，`RunAtLoad = true`，不使用 `KeepAlive`；Quit 後不在同一登入 Session 自動重啟。
- Static／Build／Install Validation：macOS static packaging validation、PyInstaller App build、structural codesign verification、component plist、`pkgbuild`、`productbuild`、PKG expansion／metadata／payload validation 與 Product Host lifecycle regression 29 項全部 PASS。
- Jamie Manual Validation：安裝、LaunchAgent、Package Receipt `0.6.2`、Menu Bar、Version `0.6.2`、無 Dock Icon、無 Terminal Window、`/ready`、Photoshop 2026 去背、WebP／PNG mismatch isolation、後續素材 continuation、Processed PNG／report／Matching／Review 全部 PASS。
- Artifact Integrity：PKG SHA-256 為 `32b55924db02074c35292a03edd564f3b98032e74565e4f1f310c8fbe50dd596`。Repository、App bundle 與 PKG payload 的 `remove-background.jsx` byte-for-byte 一致，共同 SHA-256 為 `796680519c488f53587d385a06785d3aac6059e4530953c598ec6e56ee8e89a7`。
...
- Credential-dependent validation：本機沒有有效 Developer ID Application／Installer identity；App 為 ad-hoc signature，PKG unsigned，未執行 Developer ID signing、notarization 或 stapling。Gatekeeper rejection 是 local unsigned artifact 的預期結果，不得寫成正式簽署或 Notarization PASS。
- Warning Summary：PyInstaller 只有既有條件式、跨平台或 optional-module warnings；SDK alignment 訊息為 informational；無 Build Error，未影響 App、PKG 或 Product Host regression。
- Scope Boundary：0.6.2 Packaging Commit 未修改共用 JSX、SPX Helper Core、Runtime Contract、macOS Adapter、HTTP／Browser API、Manifest、Pipeline State、Review Workspace、UI、`qrcode-demo`、Windows MSI、Auto Update 或 Upgrade／Uninstall Phase 範圍。
...
`skipped` 的正式 UI 名稱是「之後手動換圖」；它是目前 Project 的 Terminal State，視為 Review 已完成，不加入 Needs Rerun、不出現在第二輪 Review，也不進入任何 Photoshop Manifest。Project State v5 不新增 Object、Flag 或 version，只保存既有 record 內的 `status: "skipped"` 與 `review.decision: "skipped"`；匯出／匯入後維持終態。手動換圖保留這兩個值，不得重設為 `pending`；新 CSV 會建立全新 Project，不沿用舊 skipped。
...
Control Center UI Upgrade。控制台 Header 固定一般使用者入口順序為匯入CSV、匯入暫存、匯入素材資料夾、自動去背匯入素材；一般 UI 隱藏 Manifest / Runtime / Processed Folder 等技術術語。素材審核入口初始隱藏，只在 `FirstReview`／`SecondReview` 顯示；Photoshop 實際執行時顯示處理中，`Completed` 顯示「AI 去背完成」。中央版位下拉僅調整 display order，不修改 placementId、templateId、layoutState key 或 schema。

```text
v0.4.5
```

Review Workspace UI Upgrade。Navigator Information Architecture 簡化為檔名、Review Status、Dirty Status，移除 Role / Job ID / Slot / Asset Key / Processed Filename / Mode 等技術 Metadata；Review Summary 與 Filter（全部素材／待重新去背／去背失敗）位於 Navigator 上方；Workspace Layout 預設 Navigator + Workspace，Inspector 預設收合，選取裁切或橡皮擦時展開 Dynamic Inspector，儲存或取消後收合；Header 僅保留素材審閱／關閉。Review Decision Undo 已移除；第一輪 Decision Area 為核准／重新去背／之後手動換圖，第二輪為核准／之後手動換圖。未完成時 Close／Esc 受 guard，`Completed` 後不得重新開啟 Review Workspace。未修改 Crop / Eraser Core Logic、Undo Stack、Save Runtime Processed Asset Flow 或 Photoshop Runtime。
...
Current behavior superseding the「略過」label and repeatable rerun flow：Commit `8eefbb0924121f3a199c547186306c5eeb722a31` 將 `skipped` 的正式 UI 名稱改為「之後手動換圖」，不改 decision／state。第一輪顯示核准／重新去背／之後手動換圖；第二輪只顯示核准／之後手動換圖，`R` 與所有 decision 入口均不得在第二輪寫入 `needs_rerun`。未完成素材審核時 Close／Esc 必須阻止關閉；完成後進入 `Completed`，不得 reopen Review Workspace。
...
- **macOS Adapter**（`macos_adapter.py`）：沿用既有、未修改的 AppleScript 並呼叫兩平台共用 `remove-background.jsx`；Ready Check 改用 bundle id `com.adobe.Photoshop` 判斷是否已開啟（`application id "com.adobe.Photoshop" is running`），不再依賴容易因版本不同而變動的 process 名稱；已完成 macOS 實機驗證（Photoshop 2025）。Existing Transparency Skip 只修改共用 JSX，不修改 Adapter／AppleScript 邊界。
...
- **Naming Contract Consistency Fix**：processed 圖片實體檔名全域唯一規則為「原始素材 basename + `.png`」（例如 `商品A.jpg` / `商品A.webp` → `商品A.png`），不使用 `{assetKey}__processed.png` 或任何 jobId／role／slot 後綴；`assetKey` 只作為內部 State Identity／metadata／Manifest `assetKeys[]`，不進入實體檔名。該次 Naming Contract 修正對 `remove-background.jsx` 的唯一變更是 `getOutputFilename()` 缺少 `output.filename` 時的 fallback（改為 source basename + `.png`）；後續 Existing Transparency Skip 見上方 Current Status。人工「匯入處理結果」的比對邏輯也已同步改為 basename 比對（見 `docs/Photoshop Asset Pipeline.md`）。
...
2. 提供素材（匯入 CSV、點擊「自動去背匯入素材」並選擇素材資料夾——選擇時會一併取得後續寫入 Processed／ 所需的 readwrite 權限，只需選一次）。
3. 審閱素材。
...
10. 系統自動開啟第一次素材審閱（`FirstReview`），並自動選取第一筆素材顯示圖片，使用者不需要再手動點選；正式按鈕為核准／重新去背／之後手動換圖，三者皆保留既有 Auto Next。
...
5. 完成後自動回到第二次素材審閱（`SecondReview`），Filter 自動切到「待重新去背」，只顯示本輪重新處理的素材子集；核准第一筆後自動顯示下一筆，不會誤跳回完成畫面。
6. Processed／ 內同名 PNG 被正確覆蓋；本輪素材不會被 Auto Approve。
7. 第二輪只提供核准／之後手動換圖；不得顯示重新去背、不得由 `R` 快捷鍵或其他 decision 入口寫入 `needs_rerun`，也不得建立第三輪 Rerun。
8. 本輪所有素材完成 decision 後進入 `Completed`，返回控制台並顯示「AI 去背完成」；不得重新開啟 Review Workspace 或重新去背。
```

使用者可見 UI 只使用工作語言：處理中、素材處理完成、素材審閱、核准、重新去背、之後手動換圖、待重新去背、重新去背素材（N）、AI 去背完成、去背失敗、請回控制台手動更換圖片，以及 Ready Check 未通過或處理失敗時的「Photoshop 已關閉」「重新檢查」「無法寫入處理結果」「重新授權」「無法開啟素材審閱」「重試」等復原提示。不暴露 Manifest、Runtime、Processed Folder、executionId 等技術詞彙。「素材處理完成」後直接自動進入素材審閱，不存在獨立的「等待審閱」中繼狀態。
...
**部分素材去背失敗（去背失敗獨立分類，Bug Fix）**：只要至少一張素材成功，就不再顯示整批 Recovery Banner、也不再需要整批重新開始。系統直接把成功的素材帶入素材審閱；從未成功過的素材標記為「去背失敗」，於素材審閱顯示原圖與提示文字「此素材去背失敗，請回控制台手動更換圖片。」，不提供核准／重新去背／撤回按鈕；已成功過但 Rerun 又失敗的素材維持原本的 `needs_rerun`，沿用上一次成功的處理結果，不新增額外狀態。Review Workspace 圖片載入的正式 fallback 已由 Commit `c21c79e5e762598a35d6368fff5013ffd6ee21df` 修正：有 Processed 時仍預設顯示 Processed；沒有 Processed 或 Processed resolver 失敗時，只要 Original 成功就建立 Editor 並直接顯示 Original。
...
## Windows SPX Helper 0.6.2 packaging completion (2026-07-30)

Windows Phase 2 packaging is PASS. Packaging Commit: `2099e22d9e2c7bdb685d903c893dded94f933b2f` (`chore: package Windows Helper 0.6.2`); corresponding functionality Commit: `f37d37e13b0770acab0559dae318576c82458971` (`fix: guard WebP content with PNG extension`). The official PyInstaller onedir bundle and `SPX Helper-0.6.2-x64.msi` were produced. Jamie Manual Validation passed MSI Install, Apps & Features version `0.6.2`, Helper Ready, and Photoshop 2026 Remove Background.

MSI identity is Product Name `SPX Helper`, Product Version `0.6.2`, and unchanged UpgradeCode `{0E9BD5FB-A6F1-472B-8B6B-A395BDEDC941}`. Bundle and MSI payload include `photoshop/remove-background.jsx`; repository and bundle match after CRLF-to-LF normalization at SHA-256 `796680519c488f53587d385a06785d3aac6059e4530953c598ec6e56ee8e89a7`, and MSI File table confirms the JSX entry. The shipped JSX contains the pre-`app.open()` Guard for a `.png` filename with `RIFF....WEBP` magic bytes. The mismatched asset now reports through the existing per-item failure path as `background_removal_failed`; later assets continue and Review Workspace retains its existing failure flow. Windows Photoshop 2026 no longer crashes for this input.

Build completed with Error 0; WiX emitted only the existing WIX1076 / ICE90 warning, and PyInstaller optional-module warnings did not affect the bundle. Existing Transparency Skip remains included. Phase 2 architecture was not redesigned: Runtime Contract, Adapter architecture, WiX UpgradeCode, Installer Flow, Browser, Manifest, Pipeline State, Review Workspace, UI, `qrcode-demo`, and Mac PKG are unchanged. WiX `obj/` remains an ignored intermediate. No new Tag or GitHub Release was created, the MSI was not uploaded, and local commits remain unpushed.
