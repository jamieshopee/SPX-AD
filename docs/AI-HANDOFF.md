...
- SPX Helper Runtime Productization — Phase 1 Foundation：完成正式 Product Host、Running／Working／Attention、單一 Helper Instance、固定 Tray／Menu Bar、Open SPX BN Generator、About、Version、Restart 與 Quit，功能 Commit `51c4828`（`feat: add SPX Helper product foundation`）。Browser／Platform Validation 與 Jamie Manual Validation 均 PASS。Phase 1 未修改 SPX Helper Core、Runtime Contract、Browser API 或 Frontend；Installer、MSI、PKG 與登入自動啟動尚未完成。
...
Photoshop Automation、AI Workflow、Render Context & Export Workflow、QR Code、SPX Helper Core 與 SPX Helper Runtime Productization Phase 1 Foundation 均已完成。Phase 1 功能 Commit 為 `51c4828`，Browser／Platform Validation 與 Jamie Manual Validation 均 PASS；Product Host 只組合既有 Core，未修改 Runtime Contract、Browser API、Frontend 或 Photoshop 執行路徑。Branch 為 `main`，下一步是 **Phase 2 — Windows Packaging（Not Started）**。Installer、Windows MSI、macOS PKG、登入自動啟動、更新、解除安裝、簽章與 release packaging 尚未完成。目前不宣稱支援所有 Photoshop 版本；已實機驗證版本僅 Photoshop 2025。
...
- SPX Helper Runtime Productization — Phase 1 Foundation（Completed）

Next：

- Phase 2 — Windows Packaging（Not Started）。僅包含已通過 Implementation Proposal 所定義的 Windows MSI、Fresh Install、安裝後立即啟動、Startup、Start Menu、Apps & Features 與 Windows Validation。
...
- 尚未完成：Phase 2 Windows Packaging、Phase 3 macOS Packaging、Phase 4 Update + Uninstall、Phase 5 Final Validation。第一版不包含 Browser Version Check、Version Compatibility UI、Auto Update 或自動 recovery。
...
以上 4 項與 SPX Helper Core 均已完成。SPX Helper Runtime Productization 另依已通過的五階段 Implementation Proposal 推進：Phase 1 Foundation 已完成，下一步為 **Phase 2 — Windows Packaging（Not Started）**；Phase 3 macOS Packaging、Phase 4 Update + Uninstall 與 Phase 5 Final Validation 尚未開始。
...
- SPX Helper Core and Productization Phase 1 Foundation are Completed. Phase 2–5, including Installer, MSI, PKG, login auto-start, update, uninstall, signing, packaging, and final product validation, must not be described as completed.
- Phase 2 must not modify SPX Helper Core, Runtime Contract, Browser API, Frontend, Photoshop Automation, AI Workflow, Review Workspace, or the locked production architecture.
