...
- SPX Helper Runtime Productization — Phase 1 Foundation：完成正式 Product Host、Running／Working／Attention、單一 Helper Instance、固定 Tray／Menu Bar、Open SPX BN Generator、About、Version、Restart 與 Quit，功能 Commit `51c4828`（`feat: add SPX Helper product foundation`）。Browser／Platform Validation 與 Jamie Manual Validation 均 PASS。Phase 1 未修改 SPX Helper Core、Runtime Contract、Browser API 或 Frontend；Windows Installer／Startup 已由 Phase 2 完成，macOS Packaging 仍待 Phase 3。
...
- SPX Helper Runtime Productization — Phase 2 Windows Packaging（PyInstaller、WiX Toolset SDK 5.0.2 MSI、Fresh Install、Startup、Start Menu、Apps & Features、Windows Validation；功能 Commit `9240504`；Phase 3～5 未完成）

目前 Active Phase：

```text
Phase 3 — macOS Packaging（Next；Not Started）
```

Photoshop Automation、AI Workflow、Render Context & Export Workflow、QR Code、SPX Helper Core、SPX Helper Runtime Productization Phase 1 Foundation 與 Phase 2 Windows Packaging 均已完成。Phase 2 功能 Commit 為 `9240504`；Windows MSI Build／Install、背景常駐及 GitHub Pages → Helper → Photoshop → Processed PNG 均已實機 PASS。Branch 為 `main`，下一步是 **Phase 3 — macOS Packaging（Not Started）**。macOS PKG、Phase 4 Update + Uninstall 與 Phase 5 Final Validation 尚未完成。目前不宣稱支援所有 Photoshop 版本；已實機驗證版本僅 Photoshop 2025。
...
- SPX Helper Runtime Productization — Phase 2 Windows Packaging（Completed）

Next：

- Phase 3 — macOS Packaging（Not Started）。僅包含已通過 Implementation Proposal 所定義的 PKG、SPX Helper.app、Login Items、Applications、Menu Bar、Notarization 與 macOS Validation。
...
- 尚未完成：Phase 3 macOS Packaging、Phase 4 Update + Uninstall、Phase 5 Final Validation。第一版不包含 Browser Version Check、Version Compatibility UI、Auto Update 或自動 recovery。
...
以上 4 項與 SPX Helper Core 均已完成。SPX Helper Runtime Productization 另依已通過的五階段 Implementation Proposal 推進：Phase 1 Foundation 與 Phase 2 Windows Packaging 已完成，下一步為 **Phase 3 — macOS Packaging（Not Started）**；Phase 4 Update + Uninstall 與 Phase 5 Final Validation 尚未開始。
...
- SPX Helper Core, Productization Phase 1 Foundation, and Phase 2 Windows Packaging are Completed. Phase 3–5, including macOS PKG, update, uninstall, signing, and final product validation, must not be described as completed.
- Phase 3 must not modify SPX Helper Core, Runtime Contract, Browser API, Frontend, Photoshop Automation, AI Workflow, Review Workspace, or the locked production architecture.
