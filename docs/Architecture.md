...
- Project Persistence 完成：Project State v5、Persistence Layer、single-state restore 與既有 legacy project.zip import 相容；Download Complete Project 目前輸出逐 Job PNG／single-state JSON 配對。
...
13. [SPX Helper Runtime Productization — Phase 1 Foundation](#spx-helper-runtime-productization--phase-1-foundation---2026-07-17)
14. [SPX Helper Runtime Productization — Phase 2 Windows Packaging](#spx-helper-runtime-productization--phase-2-windows-packaging---2026-07-17)
15. [SPX Helper Runtime Productization — Phase 3 macOS Packaging](#spx-helper-runtime-productization--phase-3-macos-packaging---2026-07-18)
16. [SPX Helper Architecture](#spx-helper-architecturecore-completed)
17. [Photoshop Automation Architecture](#photoshop-automation-architecturecompleted)
18. [AI Workflow Architecture](#ai-workflow-architecturecompleted)
19. [QR Code Architecture](#qr-code-architecturecompleted)
20. [Documentation Structure](#documentation-structure)
21. [資料夾結構](#資料夾結構)
22. [新增 Style 流程](#新增-style-流程)
23. [維護原則](#維護原則)
...
Completed
  ...
  ├─ SPX Helper Runtime Productization — Phase 1 Foundation（功能 Commit 51c4828）
  ├─ SPX Helper Runtime Productization — Phase 2 Windows Packaging（功能 Commit 9240504）
  └─ SPX Helper Runtime Productization — Phase 3 macOS Packaging（功能 Commit ee55dd527a00361f1155ba45713ff2ce3957b06c）
...
Review Workspace（Crop / Eraser）、Photoshop Rerun Automation、Review Workspace UX Polish、Project Persistence、UI Upgrade、Photoshop Automation、AI Workflow、Render Context & Export Workflow、QR Code、SPX Helper Core，以及 Runtime Productization Phase 1 Foundation、Phase 2 Windows Packaging、Phase 3 macOS Packaging 皆已完成。Phase 3 功能 Commit 為 `ee55dd527a00361f1155ba45713ff2ce3957b06c`，macOS local app／PKG Build、Fresh Install、Login Startup 與正式 Happy Path 均 PASS。下一步為 **Phase 4 — Update + Uninstall（Not Started）**；Phase 5 尚未開始。不得將 Photoshop 2025 驗證結果延伸為支援所有 Photoshop 版本，也不得宣稱 Developer ID signing 或 Apple Notarization PASS。
...
- 匯入 single-state 或既有 legacy project.zip 後會重建 runtime `processedAssetIndex`。
...
- Download Complete Project 只輸出成功 Job 的 PNG／single-state JSON 配對，不輸出單一 Project JSON 或素材子資料夾。
...
Naming Contract（Locked by Jamie，全域唯一規則）：processed 圖片實體檔名一律是「原始素材 basename ＋ `.png`」（例如 `商品A.jpg` / `商品A.webp` 都對應 `商品A.png`）。`processedAssets` 的物件 key（上例的 `JOB001__product__slot0__商品A`）是 assetKey，只作為內部 State Identity／metadata mapping 使用；assetKey 不會、也不得出現在 `filename` 欄位或任何實體檔名裡。AI Workflow、人工備援流程（匯出處理檔／匯入處理結果）、Project Persistence（`single-state.json`）與 Review Workspace Save 共用同一套規則，Project Persistence 不會另外重新命名 processed 圖片，也沒有獨立的第二套命名邏輯。
...
- 每份 JSON 都是 version 5、`type = single`，只包含對應 Job，並沿用 single-state 的 processed image `dataUrl` 與完整還原資料。
- ZIP 根目錄不包含單一 Project JSON、Assets、Processed、Thumbnail、Hidden、Manifest 或其他子資料夾。
- 從 ZIP 取出任一 JSON，可由既有 single-state Import 重建該 Job 的 runtime source。
...
Phase 1 Product Host 是既有 SPX Helper Core 的桌面 lifecycle 外層，不取代、不複製也不修改 Core：

```text
Tray / Menu Bar（Running / Working / Attention）
  ↓ lifecycle commands
Product Host（spx_helper_product.py）
  ↓ compose unchanged Core
SPX Helper Core（spx_helper.py；127.0.0.1:8901）
  ↓ existing Runtime Contract
Existing RuntimeCore → Platform Adapter → Photoshop
```

- Product Host 啟動時建立既有 RuntimeCore 與 Helper server；Idle 時顯示 Running，Platform Adapter 執行期間顯示 Working，啟動失敗顯示 Attention。
- 固定 `127.0.0.1:8901` 與既有 Ready 邊界共同保證同一工作階段只有一個 Helper；第二次啟動不建立第二個 RuntimeCore 或 listener。
- Tray／Menu Bar 僅提供狀態、Open SPX BN Generator、About、Version、Restart SPX Helper、Quit SPX Helper，不包含設定中心、Log Viewer 或診斷介面。
- Quit 只停止本次登入期間的 Product Host，不解除安裝、不移除未來由 Packaging Phase 建立的登入自動啟動；Working 時先警告。
- Restart 先停止舊 Helper 與 listener，再啟動新 Product Host。Manual Validation 已確認 PID `6818` → `9905`，舊 Process 停止且只有一個新 Process／listener。
- Browser／Platform Validation 與 Jamie Manual Validation 均 PASS；既有 Photoshop Happy Path 正常完成。
- Installer、Windows MSI、macOS PKG、登入自動啟動、Update、Uninstall、Signing 與 Final Validation 均不在 Phase 1 完成範圍。

## SPX Helper Runtime Productization — Phase 2 Windows Packaging - 2026-07-17

Phase 2 將 Phase 1 的既有 Product Host 封裝為 Windows 正式產品，不修改 Foundation lifecycle 或 SPX Helper Core：

```text
WiX MSI（per-machine）
  ↓ install
PyInstaller executable bundle（spx_helper_product.py）
  ↓ unchanged Product Host lifecycle
SPX Helper Core → RuntimeCore → Windows Adapter → Photoshop
```

- PyInstaller 封裝既有 `spx_helper_product.py`，不建立第二套 Product Host。
- WiX Toolset SDK 固定為 `5.0.2`；WiX v7 的 OSMF 條件不符合本專案不需額外付費的 Packaging 要求，因此不採用。
- MSI 提供 per-machine Fresh Install、安裝完成後立即啟動、Windows Run 登入自動啟動、Start Menu Shortcut 與 Apps & Features 標準安裝資訊。
- Packaging source 位於 `tools/photoshop-automation/packaging/windows/`；`.venv`、`build`、`dist`、`__pycache__`、MSI 與 `wixpdb` 不納入版本控制。
- Windows Packaging configuration、Product Foundation、PyInstaller EXE、WiX MSI Build／Install、背景常駐、System Tray 與 Running 狀態均 PASS。
- Jamie Manual Validation 已完成正式流程：GitHub Pages → SPX Helper → Photoshop → Processed PNG。
- 功能 Commit：`9240504`（`feat: add Phase 2 Windows packaging`）。未修改 Runtime Contract、Browser API、Frontend、Photoshop Automation 或 Phase 1 Foundation lifecycle。
- Known Issue：同一 Windows 環境中，Version／About Information Dialog 與部分 Installer Dialog 的 OK、Esc、X 關閉事件異常。此問題不影響 Packaging、Runtime、Browser API、Photoshop Automation 或正式去背流程，另案處理。
- Phase 2 完成時的下一步為 Phase 3 — macOS Packaging；該 Phase 現已完成，詳見下一節。Phase 4 Update + Uninstall 與 Phase 5 Final Validation 尚未開始。
...
macOS PKG
  ↓ fixed install path
/Applications/SPX Helper.app（PyInstaller app bundle；LSUIElement）
  ↓ unchanged Product Host lifecycle
SPX Helper Core → RuntimeCore → macOS Adapter → Photoshop

/Library/LaunchAgents/com.spxad.helper.plist
  └─ RunAtLoad = true；無 KeepAlive
```

- 正式產品資訊：Product Name `SPX Helper`、Bundle Identifier `com.spxad.helper`、Package Identifier `com.spxad.helper.pkg`、Product Version `0.6.1`。原 `0.5.5` 與既有 GitHub Release 編號衝突，因此以新增修正 Commit 更正為 `0.6.1`，不改寫已 Push Git 歷史；目前最新既有 Release 為 `v0.6.0`，預計新 Tag／Release 為 `v0.6.1`。
...
- Helper Core 不包含 Product Host lifecycle、installer、OS login auto-start、update、uninstall、code signing 或執行中工作恢復；Phase 1 只在 Core 外新增 Product Host lifecycle。
...
- Existing Transparency Skip 功能本身未修改 Browser、Python Runtime、macOS／Windows Adapters、Manifest／Runtime API 或 Packaging Flow。SPX Helper 正式交付版本已修正為 `0.6.1`，新版 App 與 `SPX Helper-0.6.1.pkg` 均包含此 JSX；本輪只重新執行 Local Packaging、不安裝新版 PKG。既有安裝與 Manual Validation 結果維持 PASS，Bundle ID、Package ID 與 LaunchAgent 不變。
...
`job.qrCodeUrl` 為一般字串欄位，隨 `serializeJobBase()` / `createJob()` 直接保存與還原；正式 single-state JSON 與下載完整專案內每個 Job 的 single-state JSON 都會保存並正確恢復最後修改的網址。不使用 `_embeddedAssets`、`processedAssetIndex` 或 Approved Asset Resolver。Project State schema 版號未變（仍為 `version: 5`），`qrCodeUrl` 為向下相容的新增欄位。

### 匯出

下載完整專案依序透過既有 `selectJob()` Canvas transaction 套用每個 Job 的 `qrCodeUrl`，待 QR Code 與其他素材完成後才建立該 Job 的 PNG／single-state JSON 配對；空值或非法網址不顯示 QR Code，Banner 正常 Render。下載單張圖檔（直接截圖）不需額外處理，QR Code 已在畫面上即為正確結果。
