...
- Phase 3 macOS Packaging Bug Fix Commit `781df79c232a9644cc0bd69653e390ef70d12964`（`fix: launch macOS helper through LaunchAgent`）移除 PKG `postinstall` 的直接 `/usr/bin/open` 啟動，改由已 bootstrap 的 LaunchAgent job 以非強制 `kickstart` 啟動。Clean Install 後 Helper environment 不再含 `PKInstallSandbox`／installer variables，正式 GitHub Pages → Helper → Photoshop → Processed PNG 與 Jamie Manual Validation 均 PASS；`RunAtLoad = true`、無 `KeepAlive`、Quit 與 Login Startup 行為不變。
