...
- Review Workspace UI Upgrade 完成：更新素材審閱操作流程（Navigator、Dynamic Inspector、Decision Area、Completion Screen、Completion Recovery），並補充 Future AI Workflow 的最終使用者流程方向。
...
15. [Future AI Workflow 使用者流程（未來目標）](#future-ai-workflow-使用者流程未來目標)
...
Review Summary（進度、核准數、重新去背數）與 Filter 固定在 Navigator 上方，並排顯示：
...
完成判斷使用全域可審閱素材，不是目前 Filter 的結果；若切到「待重新去背」但該 Filter 目前是空的、而全域仍有未審閱素材，會顯示「目前沒有待重新去背的素材」，不會誤顯示成全部完成。
...
`重新去背素材（N）` 代表目前需要重新去背的素材數量。`N=0` 時不可執行。此入口目前只呼叫既有 `exportPhotoshopRerunManifest` callback，沿用既有 Photoshop Rerun Automation；一般 UI 不顯示 Photoshop / Manifest 等技術術語。自動化尚未實作，未來將拆分兩個 Phase 負責：Photoshop 端的 Ready Check / 批次處理 / 狀態回報屬於「Photoshop Automation」Phase；Control Center 端的自動 Import Processed Result、自動開啟第二輪 Review 屬於「AI Workflow」Phase（依賴 Photoshop Automation 先完成，見下方章節）。系統不會自動啟動 Photoshop App，使用者需自行先開啟 Photoshop。

重新去背完成後，使用「匯入處理結果」帶回新的處理結果。新的 processed asset 會覆蓋上一版 processed asset，但 original asset 仍保留。匯入後會回到 Review Workspace，必須再次核准後才會進入成品渲染。
...
## Future AI Workflow 使用者流程（未來目標）

以下為「AI Workflow」Phase 的目標使用者流程，是 Future 方向，尚未實作，不是目前行為。目前實際操作請見上方「素材審核 / 素材審閱」章節。以下內容屬於 Current Product Decisions（已由 Jamie 決定的產品方向），不代表 Proposal 已 Freeze。

AI Workflow 依賴「Photoshop Automation」Phase 先完成（目前 Active Phase，Proposal 階段）。AI Workflow 目前狀態為 Draft / Paused pending Photoshop Automation，在 Photoshop Automation 完成前不會實作以下流程。

核心原則：使用者只需在開始前自行開啟 Photoshop；通過 Photoshop Ready Check 後，不需要再操作 Photoshop，也不需要理解 Manifest、Runner、Processed Folder 或其他技術流程。Photoshop 只是背景素材處理引擎，不是使用者日常操作介面。
...
6. 若未準備完成，顯示「請先開啟 Photoshop」與「重新檢查」；使用者開啟 Photoshop 後按「重新檢查」即可繼續，不需要重新選擇 CSV / 素材資料夾。
7. Ready Check 通過後，系統進入 Processing Mode：素材處理中 18 / 63，完成後將自動開啟素材審閱。
8. 背景處理期間 Control Center 不可操作（不可修改文字、不可切換工單、不可下載、不可開始新的工作）。
9. 完成後顯示：素材處理完成（停留約 0.5～1 秒，此時間只屬於 UI 轉場，不是處理完成的判定依據）。
10. 系統自動開啟素材審閱，使用者不需要再按「開啟素材審核」：核准 / 重新去背 / 裁切 / 橡皮擦（UI 與目前完全相同）。
11. Needs Rerun = 0 → 返回控制台。
12. Needs Rerun > 0 → 點擊「重新去背素材（N）」，系統再次進入相同的 Processing Mode（Photoshop 全程保持開啟，不重新啟動），完成後自動回到素材審閱，Filter 自動切到「待重新去背」，只顯示本輪重新處理的素材。
13. 重複至 Needs Rerun = 0，Workflow 結束。
```

使用者始終只看到工作語言（素材處理中、素材處理完成、素材審閱、核准、重新去背、待重新去背、重新去背素材（N）），以及 Ready Check 未通過時的「請先開啟 Photoshop」前置提示；不會看到 Manifest、Runner、Processed Folder、Watcher、Heartbeat 等技術詞彙。「素材處理完成」後直接自動進入素材審閱，不存在獨立的「等待審閱」中繼狀態。

Photoshop Automation 與 AI Workflow 責任不重疊：Photoshop 端的 Ready Contract、批次去背、Progress / Completion 狀態回報屬於 Photoshop Automation；Control Center 端的 Manifest 建立與送出、Processing Mode、自動 Import、自動開啟 Review Workspace 屬於 AI Workflow。AI Workflow 只負責把 Control Center 端流程自動化並接到目前已完成的 Review Workspace UI，不會重新設計 Navigator、Dynamic Inspector、Decision Area 或 Completion Screen。
