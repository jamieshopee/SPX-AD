...
`重新處理素材（N）` 代表目前需要重新處理的素材數量。`N=0` 時不可執行。此入口沿用既有 Photoshop Rerun Automation，但一般 UI 不顯示 Photoshop / Manifest 等技術術語。

重新處理完成後，使用「匯入處理結果」帶回新的處理結果。新的 processed asset 會覆蓋上一版 processed asset，但 original asset 仍保留。匯入後會回到 Review Workspace，必須再次核准後才會進入成品渲染。
