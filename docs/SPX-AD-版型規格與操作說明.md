...
`重新去背素材（N）` 代表目前需要重新去背的素材數量。`N=0` 時不可執行。此入口目前只呼叫既有 `exportPhotoshopRerunManifest` callback，沿用既有 Photoshop Rerun Automation；一般 UI 不顯示 Photoshop / Manifest 等技術術語。Background Runner、Photoshop 自動啟動、自動 Import Processed Result、自動第二輪 Review 尚未實作，屬於下一個 Planned Phase「AI Workflow」（見下方章節）。
...
以下為下一個 Planned Phase「AI Workflow」的目標使用者流程，是 Future 方向，尚未實作，不是目前行為。目前實際操作請見上方「素材審核 / 素材審閱」章節。

核心原則：一般使用者完全不需要知道 Photoshop 的存在，Photoshop 只是背景素材處理引擎。

```text
1. 匯入 CSV。
2. 選擇素材資料夾。
3. 控制台自動在背景啟動素材處理（不需要手動匯出 Manifest 或執行 Runner）。
4. 控制台顯示：素材處理中（18 / 63）→ 素材處理完成 → 等待審閱。
5. 系統自動帶入處理結果（不需要手動 Import Processed Folder）。
6. 直接進入素材審閱：核准 / 重新去背 / 裁切 / 橡皮擦（UI 與目前完全相同）。
7. Needs Rerun = 0 → 返回控制台。
8. Needs Rerun > 0 → 點擊「重新去背素材（N）」，系統背景自動重跑並自動回到「待重新去背」第二輪審閱。
9. 重複至所有素材完成。
```

使用者始終只看到工作語言（素材處理中、素材處理完成、等待審閱、素材審閱、核准、重新去背、待重新去背、重新去背素材（N）），不會看到 Photoshop、Manifest、Runner、Processed Folder 等技術詞彙。

AI Workflow 只負責把背景處理自動化並接到目前已完成的 Review Workspace UI，不會重新設計 Navigator、Dynamic Inspector、Decision Area 或 Completion Screen。
