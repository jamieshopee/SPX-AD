...
- SPX Helper formal delivery version was corrected to Product Version `0.6.1` and locally repackaged as `SPX Helper-0.6.1.pkg`; the new PKG is not installed in this round. Version `0.5.5` conflicted with the existing GitHub `v0.5.5` Release while the latest existing Release was already `v0.6.0`, so additive correction commits were used without rewriting pushed history. Repo, App Bundle, and PKG Payload SHA-256 remain `467bb19a80850c6a63fe1a40d469727ea396b1d629019b934cffc7e905b3f8fc`. Existing installation and Helper startup validation, Existing Transparency, JPG, opaque PNG, Logo copy, Runtime Contract, and Packaging results remain PASS. Bundle ID, Package ID, and LaunchAgent remain unchanged.
