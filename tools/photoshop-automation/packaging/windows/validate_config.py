...
def validate_wix(packaging_dir):
    ...
    check("MSI output name includes the fixed version", "SPX Helper-0.6.2-x64" in project_text)
...
def validate_build_inputs(packaging_dir, automation_dir):
    ...
    check("Product Host version remains 0.6.2", read_product_version(product_host) == PRODUCT_VERSION)
    check(
