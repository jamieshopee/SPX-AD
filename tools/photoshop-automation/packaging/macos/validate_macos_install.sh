...
case "$MODE" in
  validate)
    ...
    [[ "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' "$APP_PATH/Contents/Info.plist")" == "0.6.1" ]] || fail "Installed application version is 0.6.1"
    pass "Installed application version is 0.6.1"
    ...
    [[ "$receipt" == *"version: 0.6.1"* ]] || fail "SPX Helper package receipt version is 0.6.1"
    pass "SPX Helper package receipt version is 0.6.1"
    assert_single_helper
