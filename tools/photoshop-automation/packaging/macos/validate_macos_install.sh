...
case "$MODE" in
  validate)
    ...
    [[ "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' "$APP_PATH/Contents/Info.plist")" == "0.6.2" ]] || fail "Installed application version is 0.6.2"
    pass "Installed application version is 0.6.2"
    ...
    [[ "$receipt" == *"version: 0.6.2"* ]] || fail "SPX Helper package receipt version is 0.6.2"
    pass "SPX Helper package receipt version is 0.6.2"
    assert_single_helper
